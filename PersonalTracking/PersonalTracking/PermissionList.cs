...
namespace PersonalTracking
{
    public partial class PermissionList : Form
    {
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            else if (detail.Sate == PermissionStates.Approved || detail.Sate == PermissionStates.Disapproved)
                MessageBox.Show("You can not update this permission");
        ...
        private void PermissionList_Load(object sender, EventArgs e)
        {
            ...
            if(!UserStatic.isAdmin)
            {
                ...
                btnDisapprove.Hide();
                btnDelete.Hide();
                btnClose.Location = new Point(430, 28);
            }
        }
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            if (rbStart.Checked)
                list = list.Where(x => x.StartDay < Convert.ToDateTime(dpStart.Value) &&
                x.StartDay > Convert.ToDateTime(dpStart.Value)).ToList();
            else if (rbEnd.Checked)
                list = list.Where(x => x.EndDay < Convert.ToDateTime(dpFinish.Value) &&
                x.EndDay> Convert.ToDateTime(dpFinish.Value)).ToList();
            if (cmbState.SelectedIndex != -1)
                list = list.Where(x => x.Sate == Convert.ToInt32(cmbState.SelectedValue)).ToList();
        ...
        private void dataGridView1_RowEnter(object sender, DataGridViewCellEventArgs e)
        {
            ...
            detail.StartDay = Convert.ToDateTime(dataGridView1.Rows[e.RowIndex].Cells[8].Value);
            detail.EndDay = Convert.ToDateTime(dataGridView1.Rows[e.RowIndex].Cells[9].Value);
            detail.Explanation = dataGridView1.Rows[e.RowIndex].Cells[13].Value.ToString();
            detail.UserNo = Convert.ToInt32(dataGridView1.Rows[e.RowIndex].Cells[1].Value);
            detail.Sate = Convert.ToInt32(dataGridView1.Rows[e.RowIndex].Cells[12].Value);
